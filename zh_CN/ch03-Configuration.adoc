...
默认情况下，++AsyncExecutor++并未激活，也不会启动。要使异步执行器与Flowable引擎一同启动，进行如下配置。
...
asyncExecutorActivate这个参数使Flowable引擎在启动时启动异步执行器。
...
**默认情况下，Flowable引擎依赖不提供SFL4J绑定jar。你需要自行将其加入你的项目，以便使用所选的日志框架。**如果没有加入实现jar，SLF4J会使用NOP-logger。这时除了一条警告外，不会记录任何日志。可以从link:$$http://www.slf4j.org/codes.html#StaticLoggerBinder$$[http://www.slf4j.org/codes.html#StaticLoggerBinder]了解关于绑定的更多信息。
...
在日志中包含的信息需要例如通过日志分析器进行实时监测时，十分有用。
