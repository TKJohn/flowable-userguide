...
Flowable使用同一个starter支持Spring Boot 2.0及1.5。主要支持Spring Boot 2.0。所以监控(actuator) endpoint只支持2.0。Flowable的starter直接饮用spring boot starter，所以如果需要使用1.5版的spring boot starter，需要自行定义。
...
Flowable需要数据库来存储数据。运行上面的代码，会得到异常提示，指出需要在classpath中添加数据库驱动依赖。现在添加H2数据库依赖：
...
应用这次启动后，可以看到使用了MySQL作为数据库（也使用了HikariCP连接池框架）：
...
这是一个Spring存储组件(repository)，提供了直接可用的增删改查。我们添加了通过username查找Person的方法。Spring会基于约定自动实现它（也就是使用names属性）。
...
最后，为了使用Spring-JPA-Flowable集成，我们在流程定义中，将Person JPA对象的ID指派为任务办理人：
...
Flowable提供了Spring Boot Actuator端点，可以提供正在运行的流程的信息。
...
Flowable自动配置用于控制Spring Boot的参数与配置。参见Spring Boot参考手册中的link:$$https://docs.spring.io/spring-boot/docs/current/reference/html/howto-properties-and-configuration.html$$[Properties and Configuration(参数与配置)]。
...
flowable.mail.server.password= # 邮件服务器的登陆密码。
...
flowable.mail.server.username= # 邮件服务器的登陆用户名。如果为空，则不需要登录。

# Actuator
management.endpoint.flowable.cache.time-to-live=0ms # 响应缓存的最大时间。
...
这是Flowable提供的所有自动配置类的列表，并包括了文档及源码的连接。记得查看你的应用的状况报告，以确认具体启用了哪些功能。（使用--debug或-Ddebug或在Actuator应用中启动应用，并使用``conditions``端点）。
...
在Spring Boot配置中使用__@Bean__发布该类的实例，这样实例会在流程引擎创建前调用。
