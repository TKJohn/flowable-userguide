...
|ldap.basedn
|-
|查询用户及组的基础DN（__专有名称 distinguished name__）。如果用户及组使用不同的基础DN，请改用__user-base-dn__及__group-base-dn__。
...
这是用户所在组的LRU缓存。用于避免每次需要查询用户所在组时都访问LDAP系统。。
...
|查询给定用户所在组所用的语句。
 ...
 返回LDAP中所有DN为__uniqueMember__，类为__groupOfUniqueNames__的对象。
...
|通过全名查询用户所用的语句。
 ...
 返回LDAP中所有姓或名匹配，类为__inetOrgPerson__的对象。
...
|通过ID查询用户所用的语句。
 ...
 返回LDAP中所有__uid__属性值匹配，类为__inetOrgPerson__的对象。
 上例中，使用{@link java.text.MessageFormat}即__{0}__注入用户ID。
 如果不能通过简单的语句进行查询，可以改用 `org.flowable.ldap.LDAPQueryBuilder` ，进行定制化查询。
...
每一个引擎都可以配置REST终端的基本认证。每个引擎需要单独配置，因为有可能会分开部署引擎，比如将DMN引擎与BPMN引擎部署在不同的服务器上。
